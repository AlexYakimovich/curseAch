...
set<int> increasing;
string currentMachineName = "DESKTOP-QE9Q318";
...
int broadcast(Message value)
{
	...
	for (int machineNumber = 0; machineNumber < machineNames.size(); machineNumber++)
	{
		if (machineNames[machineNumber] == currentMachineName)
			continue;
		wsprintf(pipeName, "\\\\DESKTOP-CO3NPLJ\\pipe\\demo_pipe%s", machineNames[machineNumber].c_str());     // связываемся с именованным каналом
...
int main() {
	...
	wsprintf(pipeName, "\\\\.\\pipe\\demo_pipe%s", currentMachineName.c_str());
